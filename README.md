...
| 15  | Target state: and then there is the version           | `target-net10`                           | .NET 10, isolated, all resolutions applied, plus two extra examples: startup DI validation (worker 2.x, not .NET 10 specific) and `null` out of configuration binding (new in .NET 10) |
